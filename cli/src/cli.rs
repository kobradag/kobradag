                        ...
                        if let Ok(msg) = msg {
                            match *msg {
                                ...
                                Events::ServerStatus {
                                    ...
                                    if !is_synced {
                                        if is_open {
                                            terrorln!(this, "Unable to update the wallet state - Kobrad node is currently syncing with the network...");

                                        } else {
                                            terrorln!(this, "Kobrad node is currently syncing with the network, please wait for the sync to complete...");
                                        }
                                    }
...
pub async fn kaspa_cli(terminal_options: TerminalOptions, banner: Option<String>) -> Result<()> {
    ...
    let banner =
        banner.unwrap_or_else(|| format!("Kobrad Cli Wallet v{} (type 'help' for list of commands)", env!("CARGO_PKG_VERSION")));
