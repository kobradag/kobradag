...
pub enum Prefix {
    #[serde(rename = "kobra")]
    Mainnet,
    #[serde(rename = "kobratest")]
    Testnet,
    #[serde(rename = "kobrasim")]
    Simnet,
    #[serde(rename = "kobradev")]
    Devnet,
    #[cfg(test)]
    A,
    #[cfg(test)]
...
impl Prefix {
    fn as_str(&self) -> &'static str {
        match self {
            Prefix::Mainnet => "kobra",
            Prefix::Testnet => "Kobratest",
            Prefix::Simnet => "kobrasim",
            Prefix::Devnet => "kobradev",
            #[cfg(test)]
            Prefix::A => "a",
            #[cfg(test)]
            Prefix::B => "b",
...
impl TryFrom<&str> for Prefix {
    ...
    fn try_from(prefix: &str) -> Result<Self, Self::Error> {
        match prefix {
            "kobra" => Ok(Prefix::Mainnet),
            "kobratest" => Ok(Prefix::Testnet),
            "kobrasim" => Ok(Prefix::Simnet),
            "kobradev" => Ok(Prefix::Devnet),
            #[cfg(test)]
            "a" => Ok(Prefix::A),
            #[cfg(test)]
            "b" => Ok(Prefix::B),
...
mod tests {
    ...
    fn cases() -> Vec<(Address, &'static str)> {
        ...
        vec![
            ...
            (Address::new(Prefix::Mainnet, Version::PubKey, &[0u8; 32]),      "kobrad:qqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqkx9awp4e"),
            (Address::new(Prefix::Mainnet, Version::PubKey, b"\x5f\xff\x3c\x4d\xa1\x8f\x45\xad\xcd\xd4\x99\xe4\x46\x11\xe9\xff\xf1\x48\xba\x69\xdb\x3c\x4e\xa2\xdd\xd9\x55\xfc\x46\xa5\x95\x22"), "kobrad:qp0l70zd5x85ttwd6jv7g3s3a8llzj96d8dncn4zmhv4tlzx5k2jyqh70xmfj"),
        ]
        // cspell:enable
    }
    ...
    fn test_errors() {
        // cspell:disable
        let address_str: String = "kobra:qqqqqqqqqqqqq1qqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqkx9awp4e".to_string();
        ...
        let address_str: String = format!("kobra:qqqqqqqqqqqqq{invalid_char}qqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqkx9awp4e");
        let address: Result<Address, AddressError> = address_str.try_into();
        ...
        let address_str: String = "kobra1:qqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqkx9awp4e".to_string();
        let address: Result<Address, AddressError> = address_str.try_into();
        assert_eq!(Err(AddressError::InvalidPrefix("kobra1".into())), address);

        let address_str: String = "kobraqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqkx9awp4e".to_string();
        let address: Result<Address, AddressError> = address_str.try_into();
        assert_eq!(Err(AddressError::MissingPrefix), address);

        let address_str: String = "kobra:qqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqkx9awp4l".to_string();
        let address: Result<Address, AddressError> = address_str.try_into();
        assert_eq!(Err(AddressError::BadChecksum), address);

        let address_str: String = "kobra:qqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqkx9awp4e".to_string();
        let address: Result<Address, AddressError> = address_str.try_into();
        assert_eq!(Err(AddressError::BadChecksum), address);
        // cspell:enable
    }
    ...
    pub fn test_wasm_serde_constructor() {
        let str = "kobra:qpauqsvk7yf9unexwmxsnmg547mhyga37csh0kj53q6xxgl24ydxjsgzthw5j";
    ...
    pub fn test_wasm_js_serde_object() {
        let expected = Address::constructor("kobra:qpauqsvk7yf9unexwmxsnmg547mhyga37csh0kj53q6xxgl24ydxjsgzthw5j");
        ...
        obj.set("prefix", &JsValue::from_str("kobra")).unwrap();
